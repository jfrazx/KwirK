...
bot
  ...
  .addNetwork(efnet, (err: Error, network: Irc) => {
    ...
    //  .reject(message => message.content.match(/^\+?!\w+/))
  ...
  .addNetwork(undernet, (err: Error, network: Irc) => {
    if (err) {
    ...
    network
      ...
      .reject(({ nick }: Message<Irc>) => nick === 'loudmouth');
  });
...
bot.network['freenode']
  ...
  .map(function (message: Message<Irc>) {
