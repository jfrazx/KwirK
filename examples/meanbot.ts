...
bot
  ...
  .on('message::*', function (message: Message<Irc>) {
    if (message.content.startsWith('!op')) {
      if (message.channel) {
        return (message.target as IrcChannel).op(message.user as IrcUser);
      } else {
    ...
    // or perhaps an action
      ...
      message.target.action(
        `out does ${message.user.name} at their own game, then grooves around the channel`,
      );
    }
    // a channel or private message
    else {
      message.target.say(
        `All I do is echo what you say ${message.user.name}: '${message.content}'  ... irritating isn't it?`,
      );
    }
  });
